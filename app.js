...
function saveProfile() {
    ...
    var age = document.getElementById('profile-age').value.trim();
    var height = document.getElementById('profile-height').value.trim();
    var weight = document.getElementById('profile-weight').value.trim();
...
function openProfile() {
    ...
    // Показываем ИМТ при открытии профиля
    updateBMI();
}
...
function updateBMI() {
    var heightStr = document.getElementById('profile-height').value;
    var weightStr = document.getElementById('profile-weight').value;
    var bmiBox = document.getElementById('bmi-box');

    // Если роста или веса нет — скрываем блок
    if (!heightStr || !weightStr || parseFloat(heightStr) <= 0 || parseFloat(weightStr) <= 0) {
    ...
    var heightM = parseFloat(heightStr) / 100;
    ...
    // Маркер на шкале
    // Шкала от 15 до 40, преобразуем ИМТ в проценты
    var scaleMin = 15;
    var scaleMax = 40;
    var percent = ((bmi - scaleMin) / (scaleMax - scaleMin)) * 100;
